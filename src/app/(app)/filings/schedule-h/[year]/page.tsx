      ...
      {!data.stub_count ? (
        ...
        <>
          ...
          <Card>
            ...
            <CardContent className="pb-4 space-y-2">
              {/* Official Schedule H line 13 is the STATE NAME, not a dollar
                  amount. The SUTA taxable-wage figure has no line number of
                  its own — shown as supporting context on line 14. */}
              <div className="flex items-start gap-3">
                <span className="text-[10px] uppercase tracking-wide text-muted-foreground pt-1 w-10 flex-shrink-0">
                  Line 13
                </span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm">Name of state where you paid unemployment contributions</p>
                </div>
                <span className="text-sm flex-shrink-0">New York</span>
              </div>
              <LineRow
                line="14"
                label="State unemployment tax paid (SUTA)"
                value={data.suta_paid}
                hint={`Paid via NYS-45 · rate ${parseFloat((sutaRate * 100).toFixed(3))}% on ${formatCurrency(data.suta_wages)} taxable wages (capped at $${Number(rates.suta_wage_base).toLocaleString()} NY SUTA wage base)`}
              />
            </CardContent>
          </Card>
